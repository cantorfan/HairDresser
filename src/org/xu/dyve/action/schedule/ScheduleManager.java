...
public class ScheduleManager extends HttpServlet {
				...
				if (request.getParameter("optype") != null) {
							...
							if (idNewEmployee > 0) {
								...
								if(empPermission.contains(idNewEmployee)){
									...
									if (ap != null){
										...
										//check is work time
										ArrayList result = isWorkTime(ap.getEmployee_id(), startDate, endDate);
										if((Boolean)result.get(0)){
											endDate = (java.util.Date) result.get(1);
											ap.setSt_time(DateUtil.toSqlTime(startDate));
											ap.setEt_time(DateUtil.toSqlTime(endDate));
					...
					if (operation.equals("NEW") && startDate != null && endDate != null && idNewEmployee != -1 && idService != -1)
					{
								...
								&& (empPermissionSet.size()==0 || empPermissionSet.contains(idNewEmployee)))
						{
							...
							Boolean req = false;
							ArrayList result = isWorkTime(ap.getEmployee_id(), startDate, endDate);
							if(find && serv != null && (Boolean)result.get(0)){
								endDate = (java.util.Date) result.get(1);
	 ...
	 * @param startDate
	 * @param endDate
	 * @return ArrayList; index[0] : isWorkTime, index[1]:endTime 
	 */
	private ArrayList<Object> isWorkTime(int employee_id, java.util.Date startDate,
		...
		int week_day = DateUtil.getDayOfWeek(startDate);
//		if (week_day == 0)
//			week_day = week_day + 7;
...
//			week_day = 6;
		...
		WorkingtimeEmp workingtimeEmp = null;
		ArrayList<WorkingtimeEmp> wtimeList = (ArrayList<WorkingtimeEmp>) WorkingtimeEmp.findByEmpIdAndDate(employee_id, DateUtil.toSqlDate(startDate));
		Employee employee = Employee.findById(employee_id);
		if(employee.getOneday() && wtimeList.size()>0){
			workingtimeEmp = wtimeList.get(0);
		}else{
			workingtimeEmp = WorkingtimeEmp.findByEmpIdAndDay(employee_id, week_day);
		}

		long startWorkTime = workingtimeEmp.getH_from().getHours()*60+workingtimeEmp.getH_from().getMinutes();
		long endWorkTime = workingtimeEmp.getH_to().getHours()*60+workingtimeEmp.getH_to().getMinutes();
		...
		if(startWorkTime<=startTime){
			result = true;
			if(endWorkTime<endTime){
				Calendar cal1 = Calendar.getInstance();
				cal1.setTime(endDate);
				cal1.set(Calendar.HOUR_OF_DAY, workingtimeEmp.getH_to().getHours());
				cal1.set(Calendar.SECOND, workingtimeEmp.getH_to().getSeconds());
				cal1.set(Calendar.MINUTE, workingtimeEmp.getH_to().getMinutes());
				cal1.set(Calendar.MILLISECOND, 0);
				resultTime = cal1.getTime();
			}
		}
		results.add(result);
