...
var Custom4 = {
	init: function() {
		...
		for(a = 0; a < inputs.length; a++) {
			if((inputs[a].type == "checkbox" || inputs[a].type == "radio") && inputs[a].className == "styled4") {
	...
	clear: function() {
		...
		for(var b = 0; b < inputs.length; b++) {
			if(inputs[b].type == "checkbox" && inputs[b].checked == true && inputs[b].className == "styled4") {
				inputs[b].previousSibling.style.backgroundPosition = "0 -" + checkboxHeight2*2 + "px";
			} else if(inputs[b].type == "checkbox" && inputs[b].className == "styled4") {
				inputs[b].previousSibling.style.backgroundPosition = "0 0";
			} else if(inputs[b].type == "radio" && inputs[b].checked == true && inputs[b].className == "styled4") {
				inputs[b].previousSibling.style.backgroundPosition = "0 -" + radioHeight4*2 + "px";
			} else if(inputs[b].type == "radio" && inputs[b].className == "styled4") {
				inputs[b].previousSibling.style.backgroundPosition = "0 0";
			}
		}
	},
