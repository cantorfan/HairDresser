...
<%
    ...
    if (!filter.equals(""))
        filter = " where " + filter;
    //new filter
    filter = " where "+Reconciliation.GIFTCARD_PAY+"='"+id+"'";
    list_trans = Reconciliation.findByFilter(filter + order);
    ...
    CashDrawing cd = null;
    CashDrawing _cd = null;

%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<title>View Giftcard history</title>
    ...
    function deleteTrans(id_trans)
    {
        new Ajax.Request( '../chkqry?rnd=' + Math.random() * 99999, { method: 'get',
                            parameters: {
                                action: "deleteTransactionfromAdmin",
                                id: id_trans
                            },
				...
				<td style="vertical-align: top;">
                    ...
                    <div class="padder">
                        ...
                        <tr class="filter">
                            ...
                            <th class="sku" style="text-align:center;" title="Payment">Action</th>
                        </tr>
                        <%
                        ...
                        <TR >
                            ...
                            <TR >
                                ...
                                <TD style="background-color:<%=bc%>">
                                    <%
                                        cd = CashDrawing.findByDate(Integer.parseInt(loc), DateUtil.parseSqlDate(tran.getCreated_dt().toString()));
                                        _cd = CashDrawing.findByDateStatus(Integer.parseInt(loc), DateUtil.parseSqlDate(tran.getCreated_dt().toString()), 0);
                                        if (_cd != null)
                                    ...
                                    %>
                                    <a href="../checkout.jsp?ct=<%=tran.getCode_transaction()%>&idc=<%=tran.getId_customer()%>&idt=<%=tran.getId()%>&tp=-1"><img src="../img/rec_view_button.png" border="0"/></a>
                                    <a href="../checkout.jsp?ct=<%=tran.getCode_transaction()%>&idc=<%=tran.getId_customer()%>&idt=<%=tran.getId()%>&tp=2&st=<%=tran.getStatus()%>"><img src="../img/rec_edit_button.png" border="0"/></a>
                                    <a href="#" <% if (!isEndDayAvailable && !isStartDayAvailable) {%>onclick="alert('You cannot delete a transaction when the day is already closed.')" <%} else {%>onclick="if(confirm('Do you really want to delete?')) deleteTrans('<%=tran.getId()%>');"<%}%>><img src="../img/rec_delete_button.png" border="0"/></a>
                                    <%
                                        isStartDayAvailable = true;
                                        isEndDayAvailable = false;
                                    %>
                                </TD>
                            </TR>
