...
//    String p_name = StringUtils.defaultString(request.getParameter("name"), "");
    ...
    ArrayList list;// = Category.findAll(offset,ActionUtil.PAGE_ITEMS);
    String filter =
//                "name LIKE '%"+ p_name +"%' AND " +
                "details LIKE '%"+ p_descr +"%' ";

    list = Category.findByFilter(filter + "LIMIT " + offset + ", " + ActionUtil.PAGE_ITEMS);
    int count = Category.countByFilter(filter);
%>
			...
			<tr>
				<td>
                    ...
                    <div class="padder">
                        ...
                        <div class="heading">
                            <h1>Manage Categories <a>(<%=count%> records)</a></h1>
                        </div>
                        <table class="data">
                        <tr>
                            <%--<th class="name" title="Category name">Name</th>--%>
                            <th class="description-category" title="Category Name">Category Name</th>
                            <th style="width:80px" />
                        </tr>
                        <TR class="filter">
                            <%--<TD class="name">--%>
...
//                            if (!cate.getDetails().toLowerCase().contains(p_descr.toLowerCase()) & !p_descr.equals("")) b = false;
                        ...
                        <TR <%if(i%2 != 0) out.print("class=\"alt\"");%>>
                            ...
                            <TD>
                                <IMG height="16" alt="Edit" title="Edit" onclick="window.location.href='./edit_category.jsp?action=edit&id=<%=cate.getId()%>'" src="../images/edit.png" width="16" longDesc="../images/edit.png">
                                <IMG title="Delete" onclick="if (confirm('Are you sure to delete?')) window.location.href='./list_category.jsp?action=delete&id=<%=cate.getId()%>&<%=ActionUtil.PAGE%>=<%=pg_num%>'" height="16" alt="Delete" src="../images/delete.png" width="16" longDesc="../images/delete.png">
                            </TD>
                        </TR><%}}%>
                        </table>
