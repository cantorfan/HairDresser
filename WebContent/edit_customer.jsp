...
<%
    ...
    if (StringUtils.isEmpty(dt))
        dt = request.getAttribute("dt").toString();
    ...
    if (StringUtils.isEmpty(id))
        id = cust!=null?String.valueOf(cust.getId()):"";
    if (StringUtils.isEmpty(id))
    ...
    {
        date = StringUtils.defaultString(cust.getDate_of_birth().toString(), "");
        Matcher lMatcher = Pattern.compile("\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}", Pattern.CASE_INSENSITIVE).matcher(date);
        if (lMatcher.matches()) {
            date = date.trim().replace('-', '/').replaceAll("/0", "/");
        }
    }
                        ...
                        <div class="padder">
                            ...
                            <form id="customer" name="customer" method="post" action="./customer.do?action=<%=action%>"  enctype="multipart/form-data" onsubmit="return formvalidate(this);">
                                ...
                                <div class="field">
                                    ...
                                    <tr>
                                    <td style="margin:0; padding: 0"><input readonly id="b_date" name="b_date" type="text" maxlength="30" value="<%=date%>"></td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <%--<input name="back" type="button" class="button_small" value="back" onclick="window.location.href='./list_customer.jsp'">--%>
                                                    <input name="back" type="button" class="button_small" value="back" onclick="window.location.href='./view_customer.jsp?id=<%=id%>&dt=<%=dt%>';">
                                            <td>
                                        </tr>
                                    </table>
                                </div>                             
