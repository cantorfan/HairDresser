...
var Custom2 = {
	init: function() {
		...
		for(a = 0; a < inputs.length; a++) {
			if((inputs[a].type == "checkbox" || inputs[a].type == "radio") && inputs[a].className.match(".*styled2.*")) {
		...
		for(a = 0; a < inputs.length; a++) {
			if(inputs[a].className.match(".*styled2.*")) {
				...
				//alert(inputs[a].name);
				if(option.length > 0){
				    //alert(option[0].innerText);
				    if(option[0].innerText != undefined)
    				    active = option[0].innerText;
                    else
                        active = option[0].childNodes[0].nodeValue;
    				textnode = document.createTextNode(active);
    				for(b = 0; b < option.length; b++) {
    					if(option[b].selected == true) {
        				    if(option[b].innerText  != undefined)
        						textnode = document.createTextNode(option[b].innerText);
                            else
                ...
                span[a].style.paddingTop = "3px";
				span[a].appendChild(textnode);
				inputs[a].parentNode.insertBefore(span[a], inputs[a]);
				//inputs[a].onchange = Custom2.choose;
				//inputs[a].addEventListener('onchange', Custom2.choose, false);
			}
		}
	},
	pushed: function() {
	...
	clear: function() {
		...
		for(var b = 0; b < inputs.length; b++) {
			if(inputs[b].type == "checkbox" && inputs[b].checked == true && inputs[b].className.match(".*styled2.*")) {
				inputs[b].previousSibling.style.backgroundPosition = "0 -" + checkboxHeight2*2 + "px";
			} else if(inputs[b].type == "checkbox" && inputs[b].className.match(".*styled2.*")) {
				inputs[b].previousSibling.style.backgroundPosition = "0 0";
			} else if(inputs[b].type == "radio" && inputs[b].checked == true && inputs[b].className.match(".*styled2.*")) {
				inputs[b].previousSibling.style.backgroundPosition = "0 -" + radioHeight2*2 + "px";
			} else if(inputs[b].type == "radio" && inputs[b].className.match(".*styled2.*")) {
				inputs[b].previousSibling.style.backgroundPosition = "0 0";
			}
		}
	},
