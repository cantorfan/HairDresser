...
<%
    ...
    String rp = StringUtils.defaultString(request.getParameter("rp"), ActionUtil.EMPTY);
...
//                                    oncl = "window.location.href='./checkout_main.jsp?dt="+dt+"';";
                            ...
                            %>
                            <div class="floatLeft submitBTN"><input name="back" type="button" class="button_small" value="back" onclick="<%=oncl%>" <%--onclick="window.location.href='./view_customer.jsp?id=<%=id%>&dt=<%=dt%>';"--%>></div> <div style="margin-left: 10px;" class="floatLeft"><h1>Customer history for <%=cust.getFname() + " " + cust.getLname() + ", Phone: " + cust.getPhone()%></h1>  </div> <div class="clear"></div>
                        </div>
                        <table class="data">
                        <tr>
